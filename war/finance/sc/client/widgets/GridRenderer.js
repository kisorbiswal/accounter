	...
	if (isc.Browser.isDOM) {
		...
		for (var rowNum = startRow; rowNum < endRow; rowNum++) {
			...
			for (colNum = startCol; colNum < endCol; colNum++) {
                if (drawRecordAsSingleCell && record && record._isGroup && hasCheckboxField  
                    ...
                    (!hasCheckboxField || colNum != checkboxColNum))
                {
                    ...
                    cellHTML[widthSlot] = (record && record._isGroup) ? groupSpanHTML 
                                                                       : colspanHTML;
				...
				if (drawRecordAsSingleCell) {
                    ...
                    if (checkboxColNum >= colNum) colNum += numCols - 
                                        (record && record._isGroup) ? 1 : 2;
                }
			}			
			// end the table row
