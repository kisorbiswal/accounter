...
isc.ValuesManager.addMethods({
    ...
    _updateValue : function (field, value, form) {
        if (form.getItem(field) == null) {
            this._itemlessValueWarning(form, field);
            return;
        }
    ...
    _itemlessValueWarning : function (member, fieldName) {
        this.logWarn("Member Form: " + member +
                 " has explicitly specified value for fieldName '" + fieldName + "', but has" +
    ...
    setValues : function (values) {
        ...
        // 'values' param now contains just the values passed in that didn't exist in any
        // member form.
        // use _updateValuesFromMembers() to add the values from every form, including any 
        // defaults for the form item values
        this.values = values;

        // grab any values defined on member that should be copied into this ValuesManager
        for (var i = 0; i < this.members.length; i++) {
            this._updateValuesFromMember(this.members[i]);
        }
    ...
    addMember : function (member) {
        ...
        if (member.getDataSource() != this.getDataSource()) {
            this.logWarn("addMember(): new member form " + member + " does not have " 
        ...
        // grab any values defined on the member that should be copied into this ValuesManager
        this._updateValuesFromMember(member);
    ...
    _updateValuesFromMember : function (member) {

        var memberValues = member.getValues(),
            items = member.getItems(),
            undef;

        // pick up any values set on the member form, unless we aleady have a value for them
        for (var fieldName in memberValues) {
            if (this.values[fieldName] !== undef) {
                this.logWarn("Member form " + member +
                    " has specified value for fieldName '" + fieldName +  
                    "' which collides with an already specified value in this " +
                    "ValuesManager. Resetting the value on the member form.");
                member.setValue(fieldName, this.values[fieldName]);
            } else if (member.getItem(fieldName) == null) {
                this._itemlessValueWarning(member, fieldName);
            } else {
                this.values[fieldName] = memberValues[fieldName];
            }
        }
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            // If values[field] from the member isn't empty, we've already copied the value
            // from the form to the VM's values above
            if (this.values[field] !== undef && memberValues[field] === undef) {
