...
setFields : function (fields) {
	...
	this.completeFields = this.bindToDataSource(fields);
    // tilegrid was crashing without this line:
    if (!this.completeFields) return;
    this.fields = isc.clone(this.completeFields);
    for (var i=0; i<this.completeFields.length; i++) {
        var item = this.completeFields.get(i);
        if (!this.fieldShouldBeVisible(item) || item.visible==false) this.fields.remove(item);
    }
},
