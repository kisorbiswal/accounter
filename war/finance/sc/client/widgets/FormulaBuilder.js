...
showHelpWindow : function () {
    ...
    if (this.helpWindow && this.helpWindow != null) {
        ...
        if (window) window.setLeft(left - (width / 2));
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (item.userFormula) {
                item._generatedFormulaFunc = item.sortNormalizer =
                    isc.FormulaBuilder.generateFunction(item.userFormula, fields);
            }
            
            if (item._generatedFormulaFunc) {
                // this is a formula - get the value of its _generatedFormulaFunc()
                record[item[fieldIdProperty]] = item._generatedFormulaFunc(record);
            } else if (item.type) 
...
// <P>
// Note that calling setSummary() will update the UI, generate the summary's funtion and 
// test it automatically.
//
