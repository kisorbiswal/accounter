 ...
 * 
 * @author Thomas Mueller and Devesh Satwani
 * 
 */
public class H2Dialect extends Dialect {
	...
	public H2Dialect() {
		...
		try {
			// HHH-2300
			Class constants = ReflectHelper
					.classForName("org.h2.engine.Constants");
		...
		registerFunction("cos",
				new StandardSQLFunction("cos", Hibernate.DOUBLE));
		registerFunction("cot",
				new StandardSQLFunction("cot", Hibernate.DOUBLE));
		registerFunction("degrees", new StandardSQLFunction("degrees",
				Hibernate.DOUBLE));
		registerFunction("exp",
				new StandardSQLFunction("exp", Hibernate.DOUBLE));
		registerFunction("floor", new StandardSQLFunction("floor",
				Hibernate.DOUBLE));
		registerFunction("log",
				new StandardSQLFunction("log", Hibernate.DOUBLE));
		...
		registerFunction("sin",
				new StandardSQLFunction("sin", Hibernate.DOUBLE));
		// registerFunction("sqrt", new StandardSQLFunction("sqrt",
		// Hibernate.DOUBLE));
		registerFunction("tan",
				new StandardSQLFunction("tan", Hibernate.DOUBLE));
		...
		registerFunction("now",
				new NoArgSQLFunction("now", Hibernate.TIMESTAMP));
	...
	private static ViolatedConstraintNameExtracter EXTRACTER = new TemplatedViolatedConstraintNameExtracter() {
		...
		public String extractConstraintName(SQLException sqle) {
			...
			if (sqle.getSQLState().startsWith("23")) {
				...
				if (idx > 0) {
					constraintName = message.substring(idx
							+ "violation: ".length());
				}
			}
