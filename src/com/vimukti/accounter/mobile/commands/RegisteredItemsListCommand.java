...
public class RegisteredItemsListCommand extends AbstractCommand {
	...
	protected void addRequirements(List<Requirement> list) {
		...
		list.add(new ShowListRequirement<FixedAsset>(getMessages()
			...
			protected String getShowMessage() {
				return getMessages().registeredItemsList();
			}
			...
			protected Record createRecord(FixedAsset value) {
				return RegisteredItemsListCommand.this.createRecord(value);
			}

			@Override
			protected void setCreateCommand(CommandList list) {
				RegisteredItemsListCommand.this.setCreateCommand(list);
			}
			...
			protected List<FixedAsset> getLists(Context context) {
				...
				for (FixedAsset asset : assets) {
					if (asset.getStatus() == FixedAsset.STATUS_REGISTERED)
						result.add(asset);
				}
				return result;
			}
	...
	protected Record createRecord(FixedAsset value) {
		...
		record.add(getMessages().bookValue(), getPreferences()
				.getPrimaryCurrency().getSymbol() + " " + value.getBookValue());
		return record;
	}
