...
public class OpenIdServlet extends HttpServlet {
	...
	UserInfo completeAuthentication(HttpServletRequest request)
		...
		receivingUrl = receivingUrl.replaceAll("http://localhost:8890",
				"https://www.accounterlive.com");
	...
	String returnTo(HttpServletRequest request) {
		return new StringBuffer(baseUrl(request)).append(
				request.getContextPath()).append(returnToPath).toString();
	}
	...
	String baseUrl(HttpServletRequest request) {
		StringBuffer url = new StringBuffer("https").append("://").append(
	...
	UserInfo onSuccess(AuthResponseHelper helper, HttpServletRequest request) {
		return new UserInfo(helper.getClaimedId().toString(), helper
				.getAxFetchAttributeValue(Step2.AxSchema.EMAIL), helper
				.getAxFetchAttributeValue(Step2.AxSchema.FIRST_NAME), helper
