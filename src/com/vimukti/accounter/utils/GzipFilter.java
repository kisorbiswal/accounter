 ...
 * <li>The content-type is in the coma separated list of mimeTypes set in the
 * <code>mimeTypes</code> initParameter or if no mimeTypes are defined the
 * content-type is not "application/gzip"</li>
 * <li>No content-encoding is specified by the resource</li>
 * </ul>
 * 
 * <p>
 * Compressing the content can greatly improve the network bandwidth usage, but
 * at a cost of memory and CPU cycles. If this filter is mapped for static
 * content, then use of efficient direct NIO may be prevented, thus use of the
 * gzip mechanism of the {@link org.mortbay.jetty.servlet.DefaultServlet} is
 * advised instead.
 * </p>
 * <p>
 * This filter extends {@link UserAgentFilter} and if the the initParameter
 * <code>excludedAgents</code> is set to a comma separated list of user agents,
 * then these agents will be excluded from gzip content.
 * </p>
 * 
 * @author gregw
 * 
...
public class GzipFilter extends UserAgentFilter {
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		boolean isComet = checkIsComet(request.getRequestURI());

		String ae = request.getHeader("accept-encoding");
		if (!isComet && ae != null && ae.indexOf("gzip") >= 0
			...
			GZIPResponseWrapper wrappedResponse = newGZIPResponseWrapper(
					request, response);
	...
	private boolean checkIsComet(String requestURI) {
		if (requestURI.contains("comet")) {
			return true;
		}
		return false;
	}
	...
	public class GZIPResponseWrapper extends HttpServletResponseWrapper {
		...
		public void setContentType(String ct) {
			...
			if ((_gzStream == null || _gzStream._out == null)
					&& (_mimeTypes == null
							&& "application/gzip".equalsIgnoreCase(ct) || _mimeTypes != null
							&& !_mimeTypes.contains(StringUtil
									.asciiToLowerCase(ct)))) {
				noGzip();
			}
		}
		...
		public ServletOutputStream getOutputStream() throws IOException {
			if (_gzStream == null) {
				...
				_gzStream = newGzipStream(_request,
						(HttpServletResponse) getResponse(), _contentLength,
						_bufferSize, _minGzipSize);
		...
		public PrintWriter getWriter() throws IOException {
			if (_writer == null) {
				...
				_writer = encoding == null ? new PrintWriter(_gzStream)
						: new PrintWriter(new OutputStreamWriter(_gzStream,
								encoding));
			}
			return _writer;
		}
	...
	public static class GzipStream extends ServletOutputStream {
		...
		public void doGzip() throws IOException {
			if (_gzOut == null) {
				...
				if (setContentEncodingGzip()) {
					_out = _gzOut = new GZIPOutputStream(
							_response.getOutputStream(), _bufferSize);
		...
		private void checkOut(int length) throws IOException {
			...
			if (_out == null) {
				if (_response.isCommitted()
						|| (_contentLength >= 0 && _contentLength < _minGzipSize))
					doNotGzip();
