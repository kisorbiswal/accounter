...
public class VendorPrePaymentGeneration {
	...
	public IContext assignValues(IContext context, IXDocReport report) {
		...
		template.setPayTo(payment.getVendor().getName());
		template.setPayFrom(payment.getPayFrom().getName());
		...
		template.setDate(payment.getDate().toString());
		template.setPaymentMethod(payment.getPaymentMethod());
		template.setChequeOrRefNo(payment.getCheckNumber());
		String symbol = payment.getCurrency().getSymbol();
		template.setRefundAmount(DataUtils.getAmountAsStringInCurrency(
				payment.getTotal(), symbol));
		context.put("paymentReceipt", template);
