...
public class TriggerInvoice implements Trigger {
		...
		// Condition for checking whether this Trigger call is for Row Updation
			...
			if (oldIsVoid != newIsVoid) {
				...
				ResultSet r = stat
						.executeQuery(new StringBuilder()
								.append(
										"SELECT COUNT(*) FROM TRANSACTION_RECEIVE_PAYMENT TRP WHERE TRP.INVOICE_ID =  ")
				...
				if (count > 0) {
					...
					stat
							.execute(new StringBuilder()
									.append(
											"UPDATE RECEIVE_PAYMENT RP SET RP.UNUSED_PAYMENTS = RP.UNUSED_PAYMENTS + (SELECT TRP.PAYMENT FROM TRANSACTION_RECEIVE_PAYMENT TRP  WHERE TRP.TRANSACTION_ID = RP.ID AND TRP.INVOICE_ID= ")
									.append(oldInvoiceId)
									.append(
											") WHERE RP.ID IN (SELECT TRP1.TRANSACTION_ID FROM TRANSACTION_RECEIVE_PAYMENT TRP1 WHERE TRP1.INVOICE_ID = ")
									.append(oldInvoiceId).append(")")
									.toString());
					...
					stat
							.execute(new StringBuilder()
									...
									.append(oldInvoiceId)
									.append(
											")  > 0 THEN SELECT  SUM(TCP2.AMOUNT_TO_USE)  FROM TRANSACTION_CREDITS_AND_PAYMENTS TCP2  JOIN CREDITS_AND_PAYMENTS CP2 ON TCP2.CREDITS_AND_PAYMENTS_ID = CP2.ID JOIN TRANSACTION_RECEIVE_PAYMENT TRP2 ON TRP2.ID = TCP2.TRANSACTION_RECEIVE_PAYMENT_ID WHERE CP2.ID = CP.ID AND TRP2.INVOICE_ID =")
									.append(oldInvoiceId)
									.append(
											" ELSE 0 END) WHERE CP.ID IN (SELECT CP1.ID FROM CREDITS_AND_PAYMENTS CP1 JOIN TRANSACTION_CREDITS_AND_PAYMENTS TCP ON CP1.ID = TCP.CREDITS_AND_PAYMENTS_ID JOIN TRANSACTION_RECEIVE_PAYMENT TRP1 ON TCP.TRANSACTION_RECEIVE_PAYMENT_ID = TRP1.ID WHERE TRP1.INVOICE_ID = ")
									.append(oldInvoiceId).append(")")
									.toString());
					...
					stat
							.execute(new StringBuilder()
									.append(
											"UPDATE TRANSACTION_RECEIVE_PAYMENT TRP SET TRP.PAYMENT = 0.0,TRP.APPLIED_CREDITS = 0.0 WHERE TRP.INVOICE_ID = ")
									.append(oldInvoiceId).toString());
					...
					stat
							.execute(new StringBuilder()
									.append(
											"UPDATE TRANSACTION_CREDITS_AND_PAYMENTS TCP SET TCP.AMOUNT_TO_USE = 0.0 WHERE TCP.ID IN (SELECT TCP.ID FROM TRANSACTION_CREDITS_AND_PAYMENTS TCP JOIN TRANSACTION_RECEIVE_PAYMENT TRP ON TCP.TRANSACTION_RECEIVE_PAYMENT_ID = TRP.ID WHERE TRP.INVOICE_ID = ")
									.append(oldInvoiceId).append(")")
									.toString());
					...
					stat
							.execute(new StringBuilder()
									...
									.append(
											" WHERE T.ID IN (SELECT TRP1.TRANSACTION_ID FROM TRANSACTION_RECEIVE_PAYMENT TRP1 WHERE TRP1.INVOICE_ID = ")
									.append(oldInvoiceId).append(")")
									.toString());
				...
				stat
						.execute(new StringBuilder()
								.append(
										"UPDATE INVOICE I SET I.PAYMENTS = I.TOTAL,I.BALANCE_DUE = 0.0 WHERE I.ID =")
								.append(oldInvoiceId).toString());
			...
			if (oldEstimateId != null) {
				...
				stat
						.execute(new StringBuilder()
								.append(
										"UPDATE ESTIMATE E SET E.IS_TURNED_TO_INVOICE = 'FALSE' WHERE E.ID = ")
	...
	private void updateStatusOfEstimate(Statement stat, int status,
		...
		stat.execute(new StringBuilder().append(
				...
				" WHERE T.ID = ").append(estimateId).toString());
	}
	...
	private void insertTaxGroupDetailsIntoTaxRateCalculationTable(
		...
		stat
				.execute(new StringBuilder()
						.append(
								"INSERT INTO TAX_RATE_CALCULATION  (TAXAGENCY_ACCOUNT_ID,RATE,TRANSACTION_ID,TRANSACTION_TYPE,TAX_CODE_ID,TOTAL,TAXABLE_AMOUNT,NON_TAXABLE_AMOUNT,TAX_COLLECTED,TAXAGENCY_ID)")
						...
						.append(" * (TR.RATE /100)")
						.append(
								",TA.ID  FROM TAXRATES TR JOIN TAXGROUP_TAXCODE TGTC ON TR.TAXCODE_ID = TGTC.TAXCODE_ID JOIN TAXGROUP TG ")
						.append(
								" ON TGTC.TAXGROUP_ID = TG.ID JOIN TAXCODE TC ON TC.ID=TR.TAXCODE_ID JOIN TAXAGENCY TA ON TA.ID = TC.TAXAGENCY_ID WHERE ")
						.append(" TG.ID = ")
						.append(newTaxGroupId)
						.append(
								" AND TR.AS_OF IN (SELECT MAX(TR1.AS_OF) FROM TAXRATES TR1 WHERE TR1.ID IN (SELECT (TR.ID) FROM TAXRATES TR JOIN ")
						.append(
								" TAXGROUP_TAXCODE TGTC ON TR.TAXCODE_ID = TGTC.TAXCODE_ID JOIN TAXGROUP TG ON TGTC.TAXGROUP_ID = TG.ID) AND ")
						.append(
								" TR1.AS_OF <= (SELECT T.T_DATE FROM TRANSACTION T WHERE T.ID=")
						.append(newInvoiceId).append(
								") GROUP BY TR1.TAXCODE_ID) )").toString());
	...
	private void updateCustomerBalance(Statement stat, Double total,
		...
		stat.execute(new StringBuilder().append(
				...
				.append(" ").append(total).append(" WHERE C.ID = ").append(
						customerId).toString());
