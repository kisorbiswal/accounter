...
public class EmployeeCategoryListDialog extends
	...
	public void initialise() {
		...
		getGrid().addRecordClickHandler(
				new GridRecordClickHandler<ClientEmployeeCategory>() {

					@Override
					public boolean onRecordClick(ClientEmployeeCategory core,
							int column) {
	...
	protected void showAddEditGroupDialog(ClientEmployeeCategory rec) {
		employeeCategory = rec;
		inputDlg = new InputDialog(this, messages.employeeCategory(), "",
				messages.employeeCategory()) {
		};

		if (employeeCategory != null) {
			inputDlg.setTextItemValue(0, employeeCategory.getName());
		}
		inputDlg.show();
	}

	@Override
	public Object getGridColumnValue(ClientEmployeeCategory group, int index) {
		switch (index) {
		case 0:
			if (group != null)
	...
	private void createCustomerGroups() {
		ClientEmployeeCategory employeeCategory = new ClientEmployeeCategory();
