package com.vimukti.accounter.web.client.externalization;

import com.google.gwt.i18n.client.Constants;
import com.google.gwt.safehtml.shared.SafeHtml;

public interface AccounterConstants extends Constants {

	public String a2Code();

	public String a3Code();

	public String a4();

	public String aboutAccounting();

	public String acceptCreditCards();

	public String accepted();

	public String account();

	public String Account();

	public String accountantTransfer();

	public String accounter();

	public String accounterCategoryList();

	public String accountFrom();

	public String accountinformation();

	public String accountInformation();

	public String accountName();

	public String AccountName();

	public String accountno();

	public String accountNo();

	public String accountNumber();

	public String accountPayable();

	public String accountRangeExceededPlsChoose0001and999();

	public String accountRangeExceededPlsChoose1001and1999();

	public String accountRangeExceededPlsChoose2001and2999();

	public String accountRangeExceededPlsChoose3001and3999();

	public String accountRangeExceededPlsChoose4001and4999();

	public String accountRangeExceededPlsChoose5001and5999();

	public String accountRangeExceededPlsChoose6001and6999();

	public String accountRangeExceededPlsChoose7001and7999();

	public String accountRangeExceededPlsChoose9001and9500();

	public String accountRangeExceededPlsChoose9501and9999();

	// FiXME

	public String accountReceivable();

	public String accountRegister();

	public String accounts();

	public String accountS();

	public String Accounts();

	public String accountsPayable();

	public String accountsReceivable();

	public String accountToDebitForSale();

	public String accountType();

	public String accountUpdationFailed();

	public String accrual();

	public String accrualBasis();

	public String accumulatedDepreciation();

	public String accumulatedDepreciationAccount();

	public String AccumulatedDepreciationAccount();

	public String accumulatedDepreciationTo();

	public String acount();

	public String actionClassNameis();

	public String active();

	public String add();

	public String Add();

	public String addanewAccount();

	// -------------
	public String addaNewBill();

	public String addaNewCustomer();

	public String addaNewCustomerRefund();

	public String addaNewInvoice();

	public String addanewPayment();

	public String addaNewPayment();

	public String addaNewQuote();

	public String addaNewsalesPerson();

	public String addaNewSupplier();

	public String addANewSupplierPayment();

	public String addaNewTaxCode();

	public String addaNewTaxItem();

	// -------------
	public String addANewVendor();

	// ------------------
	public String addANewVendorPayment();

	public String addBank();

	public String addBankAccount();

	public String addCompanyDetails();

	SafeHtml addComparativeButton();

	public String addCountry();

	public String addCountryLabel();

	public String addExpenses();

	public String addingItemTaxToTrasactionItemFailed();

	public String addingPaymenttermsToSelectitemFailed();

	public String addingPriceLevelToSelectItemFailed();

	public String addingSalesPersonToSelectitemFailed();

	public String addingShippingMethodsToSelectItemFailed();

	public String addingShippingTermsToSelectItemFailed();

	public String addingTaxGroupToSelectItemFailed();

	public String addInManagerUtilites();

	public String addInManagerUtilities();

	public String addInPacksUtilites();

	public String addInPacksUtilities();

	public String addLinks();

	public String addLogo();

	public String addNew();

	public String addNewAsset();

	public String addNewBrandingTheme();

	public String addNewCategory();

	public String addNewCustomer();

	public String addNewEmployeeExpense();

	public String addnewExpense();

	public String addNewExpense();

	public String addNewItem();

	public String addNewItm();

	public String addNewJournalEntry();

	SafeHtml addNewLine();

	public String addNewPayment();

	public String addNewSalesPerson();

	public String addNewVATCode();

	public String addNewVATGroup();

	public String addNewVATItem();

	public String addNote();

	public String addorEdit();

	public String addOrEditPayType();

	public String addOrEditVAT();

	public String addPayableInvoice();

	public String addPaymentTermTitle();

	public String addPaymentTermTitleDesc();

	public String addReceivableInvoice();

	public String addRemoveContent();

	public String address();

	public String address1();

	public String address2();

	public String addresses();

	public String addressFormLabel();

	public String addressPadding();

	public String addressPaddingValue();

	public String addWidget();

	public String adjust();

	public String adjustedPayment();

	public String adjustmentAccount();

	public String adjustmentDate();

	public String adjustments();

	public String adjustmentsToReconcileNetIncomeToNetCash();

	public String adjustPayment();

	public String adjustSalesTax();

	public String adjustSalesTaxDue();

	public String adjustVATDue();

	public String adjustVATReturn();

	public String admindefbizcom();

	public String ageing();

	public String ageing1();

	public String ageing30();

	public String ageing60();

	public String ageing90();

	public String ageingDetails();

	public String ageingforduedate();

	public String ageingfortransactiondate();

	public String agency();

	public String all();

	public String allDepreciationUpToIncluding();

	public String allErrors();

	public String allErrorsAndMessages();

	public String allExpenses();

	public String allowDocNos();

	public String alreadyAccountExist();

	public String amount();

	public String Amount();

	public String amountCanNotBeNegative();

	// -----------
	public String amountDue();

	public String amountDueToSupplier();

	public String amountDueToVendor();

	public String amountExceedsTheLimit();

	public String amountincludesVat();

	public String amountIncludesVat();

	public String amountPaid();

	public String amountReceived();

	public String amountsDueToSuppliers();

	public String amountsDueToVendor();

	public String amountShouldNotBeNegative();

	public String AmounttobeDepreciated();

	public String amounttopay();

	public String amountToPay();

	public String amountToReceive();

	public String amounttouse();

	public String amountToUse();

	public String amtDue();

	public String analysisTools();

	public String and();

	public String APAgeingDetail();

	public String APAgeingSummary();

	public String APAging();

	public String APAgingDetails();

	public String APAgingSummary();

	public String appliedCredit();

	public String appliedCredits();

	public String applyCreditAndPayments();

	public String applyCreditsandPayments();

	public String applyCreditsAndPayments();

	public String applyCreditsAndPaymentsFor();

	public String approve();

	public String approveButton();

	public String APR();

	public String april();

	public String ARAgeing();

	public String ARAgeingDetail();

	public String ARAgeingDetails();

	public String ARAgeingSummary();

	public String areYouRegisteredForVAT();

	public String arial();

	public String arrangeBy();

	public String asof();

	public String asOf();

	public String Asof();

	public String asset();

	public String assetAccountYouHaveSelectedNeedsLinkedAccumulatedDepreciationAccount();

	public String assetApdationFailed();

	public String assetNumber();

	public String assetNumberShouldNotBeEmpty();

	public String assetOptions();

	public String assetSold();

	public String assetType();

	public String assignVatCodesToItems();

	public String athreeCode();

	public String atwoCode();

	public String AUG();

	public String august();

	public String australia();

	public String automaticSequencing();

	public String automaticSequencingData();

	public String available();

	public String availableItems();

	public String awaitingAuthorisation();

	public String back();

	public String background();

	public String balance();

	public String balanceasof();

	public String balanceAsOf();

	public String balanceDue();

	public String balances();

	public String Balances();

	public String balanceSheet();

	public String balanceShouldBeInTheRange();

	public String bank();

	public String bankaccount();

	public String bankAccount();

	public String bankAccountInformation();

	public String bankAccountNo();

	public String bankAccountNumber();

	public String bankAccounts();

	public String bankAccountType();

	public String bankBranch();

	public String bankCategory();

	public String bankCharge();

	public String bankCreated();

	public String bankDeposite();

	public String bankFinanceCategorynumber();

	public String banking();

	public String bankingHome();

	public String bankingList();

	public String bankingLists();

	public String bankingServices();

	public String bankingSummary();

	public String bankingTransactions();

	public String bankname();

	public String bankName();

	public String bankOnline();

	public String basic();

	public String beforeYouFile();

	public String beginingBalance();

	public String belgium();

	public String billAmount();

	public String billDate();

	public String billingAddress();

	public String billNo();

	public String billPaid();

	public String billsAndItemReceipts();

	public String billsAndItemReceiptsList();

	public String billsDue();

	public String billsDueOnOrBefore();

	public String billsReceived();

	public String billsToPay();

	public String billsToReceive();

	public String billTo();

	public String bizantraFinance();

	public String bizantraPurchases();

	public String bizantraSales();

	public String blankTransIsNotAllowed();

	public String boldCell();

	public String boldFieldsShouldFill();

	public String bookValue();

	public String bottom();

	public String bottomMargin();

	public String bottomMarginValue();

	public String box3TotalVATDue();

	public String box5NetVATToPayOrReclaimIfNegative();

	public String brandingTheme();

	public String brandingThemeCombo();

	public String businessFax();

	public String businessphone();

	public String businessPhone();

	public String businessServices();

	public String businessTypes();

	public String buyChecksAndForms();

	public String calibri();

	public String cambria();

	public String canada();

	public String cancel();

	public String cancelButton();

	public String cancelled();

	public String cancelTransfer();

	public String capitalAndReserves();

	public String cardOrLoadNumber();

	public String cash();

	public String cashAccount();

	public String cashAtBeginningOfThePeriod();

	public String cashAtEndOfPeriod();

	public String cashBackAccount();

	public String cashBackAccountShouldBeSelected();

	public String cashBackAmount();

	public String cashBackAmountErrorMsg();

	public String cashBackAmountShouldnotBeGreaterthanDepositedAmount();

	public String cashBackMemo();

	public String cashBasis();

	public String cashBasisAccounting();

	public String cashDiscount();

	public String cashDiscountGiven();

	public String cashDiscountPleaseAddDetails();

	public String cashDiscountTaken();

	public String cashExpense();

	public String cashExpenses();

	public String cashFlowcategory();

	public String cashFlowCategory();

	public String cashFlowReport();

	public String cashFlowStatement();

	public String cashPurchase();

	public String cashPurchases();

	public String cashSales();

	public String category();

	public String category1099();

	public String CategoryName();

	public String categoryNumber();

	public String cCCompany();

	public String changeLogo();

	public String changePassword();

	public String changes();

	public String changeStartDate();

	public String charge();

	public String chartOfAccount();

	public String chartOfAccounts();

	public String chartOfAccountsInformation();

	public String check();

	public String checkDetail();

	public String checkIssued();

	public String checkno();

	public String checkNo();

	public String cheque();

	public String chequeNo();

	public String chooseBusinessType();

	public String chooseEndingDate();

	SafeHtml chooseLogo();

	public String chooseVATAgency();

	public String city();

	public String classNameis();

	public String clear();

	public String clearLog();

	public String clientFinanceDate();

	public String close();

	public String closeCompany();

	public String closed();

	public String closeFiscalYear();

	public String closeofFiscalYear();

	public String cm();

	public String code();

	public String codeAreNotAllowed();

	public String codeToEnterNew();

	public String collapseHierarchy();

	public String columnHeadings();

	public String comapnyIdentification();

	public String comment();

	public String comments();

	public String commissionItem();

	public String commmodityCode();

	public String companiesAndReceivable();

	public String company();

	public String companyandFinancial();

	public String companyAndFinancial();

	public String companyAndPreferences();

	SafeHtml companyCommentHtml();

	public String companyDetails();

	public String companyFile();

	public String companyinfo();

	public String companyInformation();

	public String companyLists();

	public String companyName();

	public String companyPreferences();

	public String companyPrefeTitle();

	public String companyRegistrationNumber();

	SafeHtml companySettingsTitle();

	public String companySetup();

	public String completed();

	public String compressData();

	public String confirmNewPassword();

	public String contact();

	public String contactAddressValue();

	public String contactDetails();

	public String contactDetailsHtml();

	public String contactDetailsLabel();

	public String contactname();

	public String contactName();

	public String contactNo();

	public String contactNumber();

	public String contacts();

	public String conversionAccount();

	SafeHtml conversionBalanaceHeader();

	public String conversionBalance();

	public String conversionBalanceTitle();

	public String conversionBodyComment();

	SafeHtml conversionCommet();

	public String conversionDate();

	SafeHtml conversionDateButton();

	public String conversionDateTitle();

	SafeHtml conversionHTML();

	public String copy();

	public String copyTheme();

	public String cost();

	public String costOfGoodSold();

	public String couldNotLoadQuote();

	public String country();

	public String countryCode();

	public String countryListLabel();

	public String countryName();

	public String countryRegion();

	public String countryRegionCode();

	public String countryRegionList();

	public String countryRegionName();

	public String creaditCardCharges();

	public String createCompany();

	public String createCompanyFailed();

	public String createDate();

	public String createdBy();

	public String createdSuccessfully();

	public String createFailed();

	public String createFascalYear();

	public String createForm();

	public String createInvoiceFailed();

	public String createNewAccount();

	public String createNewUser();

	public String createStatement();

	public String createTaxes();

	public String createUser();

	public String credit();

	public String creditAmount();

	public String creditAndRefunds();

	public String creditCard();

	public String CreditCard();

	public String creditCardAccountInformation();

	public String creditCardCharge();

	public String creditCardCharges();

	public String creditCardCompanies();

	public String creditCardExpense();

	public String creditCardExpenses();

	public String creditCardOrLineOfCredit();

	public String creditCardProcessing();

	public String creditlimit();

	public String creditLimit();

	public String creditLimitShouldBeInTheRange();

	public String creditLimitShouldNotBeNull();

	public String creditNo();

	public String creditNoteNo();

	public String creditNotePrefix();

	public String creditNotePrefixValue();

	public String creditNoteTitle();

	public String creditNoteValue();

	public String creditor();

	public String creditOverview();

	public String creditProfile();

	// public String StatementPeriod();
	//
	// public String allOpenTransactions();

	public String creditRating();

	public String creditRatingList();

	public String credits();

	public String creditsAndPayments();

	public String creditTo();

	public String criticalErrorsOnly();

	public String current();

	public String currentAssets();

	public String currentBalance();

	public String currentEmployee();

	public String currentHalfYear();

	public String currentLiability();

	public String currentMonth();

	public String currentQuarter();

	public String currentRate();

	public String currentView();

	public String currentYear();

	public String custom();

	SafeHtml customdocxTheme();

	public String customer();

	public String customeR();

	public String customerAddress();

	public String customerBalance();

	public String customerCredit();

	public String customerCreditMemo();

	public String customerCreditNote();

	public String customerGroup();

	public String customerGroupList();

	public String customerGroups();

	public String customerItems();

	public String customerList();

	public String customerLists();

	public String customerName();

	public String customeRName();

	public String customerNumber();

	public String customerOrderNo();

	public String customerOrderNumber();

	public String customerPayment();

	public String customerPrePayment();

	public String customerRefund();

	public String customerRefundIssued();

	public String customerRefunds();

	public String customers();

	public String customersAndReceivable();

	public String customersHome();

	public String customerSince();

	public String customersList();

	public String customerStatement();

	public String customerTransactionHistory();

	public String customerUpdatedSuccessfully();

	public String customerVATCode();

	public String custrefund();

	public String custRefund();

	public String cutomerList();

	public String cyprus();

	public String dataUtilities();

	public String date();

	// ----------------
	public String Date();

	public String date28();

	public String date29();

	public String date30();

	public String date31();

	public String dateCreated();

	public String dateDisposed();

	public String dateEffective();

	public String dateofBirth();

	public String dateofHire();

	public String dateofLastReview();

	public String dateofRelease();

	public String dateRange();

	public String dateShouldbeUnique();

	public String dateShouldBeUnique();

	public String dateShouldNotBeNull();

	public String datesold();

	public String days();

	public String Days1();

	public String Days2();

	public String Days3();

	public String days30();

	public String days60();

	public String days90();

	public String DDINumber();

	public String ddMMMyyyy();

	public String debit();

	public String debitOverview();

	public String debtor();

	public String debtors();

	public String debtorsTotal();

	public String DEC();

	public String december();

	public String declineButton();

	public String decrease();

	public String decreasePriceLevelByThisPerc();

	public String decreasePriceLevelPercentage();

	public String decreaseTaxLine();

	public String decreaseTAXLine();

	public String decreaseVATLine();

	public String deduction();

	public String defaultAccounts();

	public String defaultCash();

	public String defaultFileAsSettings();

	public String defaultPurchasePrice();

	public String defaultSellPrice();

	public String defaultWareHouse();

	public String defbiz();

	public String delete();

	public String deleteButton();

	public String deleteFailed();

	public String deleteFiscalYear();

	SafeHtml deleteHtml();

	public String deleteThemeLabel();

	public String deliverydate();

	public String deliveryDate();

	public String deposit();

	public String depositDetail();

	public String deposite();

	public String depositIn();

	public String depreciateTo();

	public String depreciation();

	public String depreciationAccount();

	public String depreciationFor2010FinancialYear();

	public String depreciationMethod();

	public String depreciationRate();

	public String depricatiedFrom();

	public String depriciationForThe();

	public String depriciationForTheYearValueFinancialYear();

	public String depth();

	public String description();

	public String descriptioN();

	public String despatchDate();

	public String detailedLog();

	public String details();

	public String detailsHighlightedInRedMustBeEntered();

	public String directDebit();

	public String directProductsMaterialCosts();

	public String discount();

	public String discountaccount();

	public String discountAccount();

	public String discountAmount();

	public String discountDate();

	public String discountPerc();

	public String discounts();

	public String disposalDate();

	public String disposalJournal();

	public String disposalPrice();

	public String disposalSummary();

	public String dispose();

	public String disposingRegisteredItem();

	public String documentNo();

	public String dollarMargin();

	public String doNotLog();

	public String downloadonlineTransaction();

	public String downloadTaxLines();

	public String doYouNeedToMakeAnAdjustment();

	public String doYoupaySalesTaxes();

	public String doYouWantPrintYourVATReturn();

	public String doyouwanttoDeletetheTransaction();

	public String doyouwanttorejecttheEstimate();

	public String doyouwanttoVoidtheTransaction();

	public String draftInvoices();

	public String due();

	public String dueDate();

	public String dueForPayment();

	public String duplicationCustomerNameNotAllowed();

	public String duplicationOf();

	public String duplicationOfAccountsIsNotAllowed();

	public String duplicationOfAssets();

	public String duplicationOfJournalEntriesNotAllowed();

	public String DuplicationOfSalesPesonNotAllowed();

	public String duplicationOfSupplierNameAreNotAllowed();

	public String duplicationOfTaxAgencyNameAreNotAllowed();

	public String duplicationOfVATCodeAreNotAllowed();

	public String duplicationOfVATGroupIsNotAllowed();

	public String duplicationOfVATItemAreNotAllowed();

	public String duplicationOfVendorNameAreNotAllowed();

	public String duplicationTaxAgencyNamesNotAllowed();

	public String duplicationVATCodeAreNotAllowed();

	public String earning();

	public String ecSalesList();

	public String ECSalesList();

	public String ECSalesListDetailReport();

	public String ecSalesListDetails();

	public String edit();

	public String editBrandThemeLabel();

	public String editCountry();

	public String editFiscalYear();

	public String email();

	public String emailAndInternet();

	public String emailFormatExpr();

	public String employee();

	public String employEe();

	public String employeeDetails();

	public String employeeExpense();

	public String employeeExpenses();

	public String employeeLeaveDate();

	public String employeeName();

	public String employeeNumber();

	public String employeeStartDate();

	public String employeeStartDateShouldBeBeforeTheEmployeeLeaveDate();

	public String employeeStatus();

	public String employeeType();

	public String empreimb();

	public String emptyValue();

	public String endDate();

	public String endingbalance();

	public String endingBalance();

	public String Endingbalance();

	public String endLastCalenderQuarter();

	public String endOfARNINC();

	public String endOfFiscalYear();

	public String endPreviousFiscalQuarter();

	public String endThisCalanderQuarter();

	public String endThisCalanderQuarterToDate();

	public String endThisCalanderYear();

	public String endThisCalanderYearToDate();

	public String endThisFiscalQuarter();

	public String endThisFiscalQuarterToDate();

	public String endThisFiscalYear();

	public String endThisFiscalYearToDate();

	public String endThisMonth();

	public String endThisMonthToDate();

	public String endThisWeek();

	public String endThisWeekToDate();

	public String endYesterday();

	public String enterAddress();

	public String enterAppropriateFiscalYear();

	public String enterBill();

	public String enterBills();

	public String enterEachIndividualVAT();

	public String enterExpenses();

	public String enterInvoice();

	public String enterNameOfCountry();

	public String enterPayments();

	public String enterTaxCode();

	public String enterValidAmount();

	public String enterYourEmail();

	public String entryNo();

	public String equity();

	public String error();

	public String errorForaddresspadField();

	public String errorForbottomMarginField();

	public String errorForTopMarginField();

	public String errorLoadingItemReceipt();

	public String errorLoadingPurchaseOrder();

	public String errorLoadingQuote();

	public String errorLoadingSalesOrder();

	public String EUVATExemptCustomer();

	public String EUVATExemptSupplier();

	public String exceedsTheLimit();

	public String exception();

	public String exit();

	public String expense();

	public String expenseAccount();

	public String expenseClaims();

	public String expenseRealtedTo();

	public String expenseReport();

	public String expenses();

	public String expensesList();

	public String expirationDate();

	public String expired();

	public String failed();

	public String failedPayments();

	public String failedToApplyChanges();

	public String failedToCreateBank();

	public String failedtoGetListofAccounts();

	public String failedToGetPurchaseOrder();

	public String failedToGetRecievePayments();

	public String failedToGetTransactionNumber();

	public String FailedToGetTransactionNumber();

	public String failedToGetVocherNumber();

	public String failedToInitializeCompanyConstants();

	public String failedToLoadBankingHome();

	public String failedToLoadCashBasisJournalEntryFailed();

	public String failedToLoadCashPurchase();

	public String failedToLoadCompanyHome();

	public String failedToLoadCompanyPreferences();

	public String failedToLoadCreditCardCharg();

	public String failedToLoadCustomerHome();

	public String failedToLoadDisposedFixedAssetView();

	public String failedToLoadSellFixedAssetView();

	public String failedToloadTaxGroup();

	public String failedToloadTaxItem();

	public String failedToLoadVendorsHome();

	public String failedToloadWriteCheck();

	public String failedToRetrieveVatBoxesForVATAgency();

	public String failedToUpdate();

	public String failedToUpdateCompany();

	public String failedtoUpdatetheCompany();

	public String failedtToLoadReportsHome();

	public String fax();

	public String FEB();

	public String february();

	public String federalTaxId();

	public String female();

	public String fetchingCompany();

	/**
	 * Menus Title
	 * 
	 */
	public String file();

	public String fileAs();

	public String fileVat();

	public String fileVAT();

	public String fileVATCreated();

	public String fileVATReturn();

	public String fileVATReturnNow();

	public String filingRecordsName();

	public String filter();

	public String Filter();

	public String filterBy();

	public String filterByBilldueonorbefore();

	public String financeCategory();

	public String financeCategoryList();

	public String financeWindow();

	public String financialAccount();

	public String financialDetails();

	public String financialYear();

	public String financialYearToDate();

	public String financialYTD();

	public String financingActivities();

	public String find();

	public String finish();

	public String firstLast();

	public String firstName();

	public String fiscalYear();

	public String fiscalYearButtons();

	public String fiscalYearsCanBeEdited();

	public String fixedAssetItemHasBeenDiposed();

	public String fixedAssetItemHasBeenSold();

	public String fixedAssetManager();

	public String fixedAssets();

	public String fixedAssetshelp();

	public String fixedAssetsList();

	public String fixedAssetsNewFixedAsset();

	public String fixedAssetsPendingItemsList();

	public String font();

	public String fontSize();

	public String fontWeigtht();

	SafeHtml footerComment();

	public String forgetPassword();

	public String formLayoutsList();

	public String france();

	public String from();

	public String fromAccount();

	public String fullName();

	public String fullTime();

	public String fullTimePartTime();

	public String fullyApplied();

	public String fundTransfered();

	public String futureEmployee();

	public String gainOnDisposal();

	public String GainsOrLosses();

	public String gender();

	public String general();

	public String generalLedgerReport();

	public String generalSettings();

	SafeHtml generalSettingsHeading();

	public String generalSettingsLabel();

	public String georgia();

	public String germany();

	public String get();

	public String getCompanyFailed();

	public String getCustomersRefundListViewHeading();

	public String getLogUpto();

	public String getNumbersandIdsTitle();

	public String getReceivedPaymentListViewHeading();

	public String getStarted();

	public String gettingStarted();

	public String getUser();

	public String getUserFailed();

	public String glReport();

	public String goToAccountReceivable();

	public String goToAccountsPayable();

	public String gotoBanking();

	public String goToExpenseCliams();

	public String greece();

	public String grossProfit();

	public String grouping();

	public String groupNameOrNumber();

	public String groupRate();

	public String groupType();

	public String gwtListBox();

	public String headings();

	public String healthcareProfessional();

	public String help();

	SafeHtml helpContent();

	public String HelpwiththisWindow();

	public String hierarchy();

	public String highlightedFormItem();

	public String history();

	public String historyList();

	public String hmCustomsExciseVAT();

	public String home();

	public String homeAddress();

	public String homeAddress1();

	public String homeBasedSales();

	public String homeEmail();

	public String homePhone();

	public String howMuchDepreciationShouldBeIncludedInFinancialYear();

	public String howMuchDepriciationShouldBeIcludedInThisFinancialYear();

	public String iBuyThisItem();

	public String ibuythisproduct();

	public String ibuythisservice();

	public String id();

	public String Id();

	public String imageAction();

	public String imageActionContainer();

	public String importBankFiles();

	public String importExcelData();

	public String importPayments();

	public String inActive();

	public String inch();

	public String income();

	public String incomeAccount();

	public String incomeAndExpenseAccounts();

	public String incomeTax();

	public String increase();

	public String increasePriceLevelByThisPerc();

	public String increasePriceLevelPercentage();

	public String increaseTaxLine();

	public String increaseTAXLine();

	public String increaseVATLine();

	public String india();

	public String indirectCosts();

	public String insuranceAgency();

	public String integrateWithBusinessContactManager();

	public String invalidAmount();

	public String invalidAmountEntetered();

	public String invalidCheckNumber();

	public String invalidCreditLimit();

	public String invalidDate();

	public String invalidDateOfBirth();

	public String InvalidDespatchDate();

	public String invalidDueDate();

	public String InvalidDueDate();

	public String invalidEntry();

	public String invalidInput();

	public String invalidOpeningBalance();

	public String invalidTransactionDate();

	public String invalidValue();

	public String inventoryItems();

	public String investingActivities();

	public String inviteUser();

	public String INVno();

	public String invoice();

	public String invoiceAmount();

	public String invoiceBranding();

	SafeHtml invoiceBrandingHTML();

	public String invoiceBrandingLabel();

	SafeHtml invoiceComment();

	public String invoiced();

	public String invoicedAmount();

	public String invoiceDate();

	public String invoiceList();

	public String invoiceNo();

	public String invoicePrefix();

	public String invoicePrefixValue();

	public String invoices();

	public String ireland();

	public String isActive();

	public String isConsiderAsPercentange();

	public String isCreated();

	public String isCreatedSuccessfully();

	public String isDeletedSuccessfully();

	public String iSellThisItem();

	public String isellthisproduct();

	public String isellthisservice();

	public String isoCode();

	public String ISOCode();

	public String isService();

	public String issued();

	public String issueDate();

	public String issuePayments();

	public String issuePaymentWith();

	// String general();

	public String isTaxable();

	public String isUpdatedSuccessfully();

	public String IsUpdatedSuccessfully();

	public String isVoid();

	public String IsVoid();

	public String isVoided();

	public String italy();

	public String item();

	public String itemCreatedSuccessfully();

	public String itemDeletedSuccessfully();

	public String itemGroup();

	public String itemGroupList();

	public String itemInformation();

	public String itemIsActive();

	public String itemList();

	public String itemName();

	public String itemPurchase();

	public String itemReceipt();

	public String ItemReceipt();

	// String taxExclusive();

	// String taxInclucive();

	public String itemReciepts();

	public String items();

	public String itemsAndExpenses();

	public String itemTax();

	public String itemTaxCode();

	public String itemType();

	public String itemupdatedSuccessfully();

	public String ITServices();

	public String iwouldliketoberunPayroll();

	public String JAN();

	public String january();

	public String january01();

	public String jobresellAccount();

	public String jobTitle();

	public String journal();

	// String paymentAdviceCutAway();

	public String journalEntries();

	public String journalEntry();

	public String journalEntryList();

	public String journalEntryNew();

	public String journalEntryNo();

	public String journalNumber();

	public String journalUpdatedSuccessfully();

	public String journalView();

	public String JUL();

	public String july();

	public String JUN();

	public String june();

	// String draftInvoiceTitle();

	// String approvedInvoiceTitle();

	public String kenya();

	public String keyFinancialIndicators();

	public String labelTitle();

	public String lableTitle();

	public String last30Days();

	public String last3Months();

	public String last45Days();

	public String last6Months();

	public String lastCalenderYear();

	public String lastDepreciation();

	public String lastFinancialQuarter();

	public String lastFinancialYear();

	public String lastFirst();

	public String lastLogin();

	public String lastMonth();

	// String point12();

	// String point13();

	// String point14();

	// String point15();

	// String exclusive();

	// String inclusive();

	public String lastName();

	public String lastVATQuarter();

	public String lastVATQuarterToDate();

	// String showPaymentAdvice();

	public String lastWeek();

	public String lastYear();

	public String latestQuote();

	public String lawFirm();

	public String left();

	public String legalCompanyName();

	// String showTaxesAs();

	// String taxType();

	public String legalInformation();

	public String legalName();

	public String lessAccumulatedDepreciationOn();

	public String liabilities();

	public String liability();

	public String liabilityAccount();

	public String linetotal();

	public String lineTotal();

	// String draftBoxValue();

	// String approvedValue();

	public String links();

	public String listOfPeriods();

	public String loadingFinancePleaseWait();

	public String loadingPurchasePleaseWait();

	public String loadingSalesPleaseWait();

	public String logContent();

	public String login();

	public String logIn();

	public String loginCount();

	public String loginDate();

	public String LoginsThisWeek();

	public String logo();

	public String logoAlignment();

	SafeHtml logoComment();

	public String logoType();

	public String logSpace();

	public String longTermLiability();

	public String lossOnDisposal();

	public String makeActive();

	public String makeDeposit();

	public String makeDeposits();

	public String makeDepostTransationsListFailed();

	public String makeInActive();

	public String male();

	public String malta();

	public String manageCountryRegionList();

	public String manageCreditCardProcessing();

	public String manageCustomerGroup();

	public String manageEmailCoverletter();

	public String manageFiscalYear();

	public String manageItemGroup();

	public String manageItemTax();

	public String manageItemTaxCode();

	public String managePaymentTerm();

	public String managePriceLevelListGroup();

	public String managerName();

	public String managerSupportLists();

	public String manageSalesGroups();

	public String manageSalesItems();

	public String manageSalesTaxCodes();

	public String manageSalesTaxGroup();

	public String manageSalesTaxGroups();

	public String manageServiceMappings();

	public String manageShippingMethodList();

	public String manageShippingTermList();

	public String manageSupplierGroup();

	public String manageSupportLists();

	public String manageUsers();

	public String manageVatCodes();

	public String manageVendorGroup();

	public String manageWordTemplates();

	public String manufacturing();

	public String manufacturingRepresentative();

	public String MAR();

	public String march();

	public String marginT();

	public String marginTop();

	public String masterCard();

	public String matchTrasactions();

	public String mauritius();

	public String maxStockAlertLevel();

	public String maxTaxLines();

	public String may();

	public String MAY();

	public String measure();

	public String measurement();

	public String measureType();

	public String memo();

	public String menuBarParent();

	public String menuItemImage();

	public String mergeAccounts();

	public String mergeCustomers();

	public String mergeFinacialAccounts();

	public String mergeFinancialAccounts();

	public String mergeItems();

	public String mergeSuppliers();

	public String minStockAlertLevel();

	public String mixedTaxLines();

	public String mobileNumber();

	public String mobilePhone();

	public String moneyComingIn();

	public String moneyGoingOut();

	public String month();

	public String mostProfitableCustomer();

	public String mostProfitableCustomers();

	public String mozabique();

	public String msg();

	public String myAccountantwillrun();

	public String myriad();

	public String name();

	public String Name();

	public String nameOrNumberIsAlreadyInUse();

	public String netAmount();

	public String netAssets();

	public String netCashChangeForThePeriod();

	public String netCashProvidedByfinancingActivities();

	public String netCashProvidedByinvestingActivities();

	public String netCashProvidedByOperatingActivities();

	public String netCurrentAssets();

	public String netherlands();

	public String netIncome();

	public String netOrdinaryIncome();

	public String netOtherIncome();

	public String netPrice();

	public String netProfit();

	public String netVATToPayOrReclaimIfNegativeBOX5();

	public String New();

	public String newAccount();

	public String newAsset();

	public String newAssetwithName();

	public String newBank();

	public String newBankCategory();

	public String newBankName();

	public String newBill();

	public String newBrandingThemeButton();

	public String newBrandThemeLabel();

	public String newCashBasisJournalEntry();

	public String newCashPurchase();

	public String newcashsale();

	public String newCashSale();

	public String newCheck();

	public String newCompany();

	public String newCreditCardCharge();

	public String newCreditMemo();

	public String newCreditNotes();

	public String newCreditRating();

	public String newCreditsAndRefunds();

	public String newCustomer();

	public String newCustomerCreated();

	public String newCustomerGroup();

	public String newCustomerPayment();

	public String newFinanceCategory();

	public String newFiscalYear();

	public String newFixedAsset();

	public String newInvoice();

	public String newInvoiceCreated();

	public String newItem();

	public String newItemGroup();

	public String newItemTax();

	public String newJournalEntry();

	public String newPassword();

	public String newPayee();

	public String newPaymentTerms();

	public String newPriceLevel();

	public String newProduct();

	public String newPurchaseOrder();

	public String newQuote();

	public String newReceivePayment();

	public String newSalesOrder();

	public String newSalesperson();

	public String newSalesPerson();

	public String newService();

	public String newServiceItem();

	public String newShippingMethod();

	public String newShippingTerms();

	public String newStartDate();

	public String newSupplier();

	public String newSupplierPrePayment();

	public String newTaxAgency();

	public String newTAXAgency();

	public String newTaxCode();

	public String newTaxGroup();

	public String newTaxItem();

	public String newTimeEntry();

	public String newvatAgency();

	public String newVATAgency();

	public String newVatCode();

	public String newVATCode();

	public String newVATCodeCreated();

	public String newvatGroup();

	public String newVATGroup();

	public String newVATGroupCreated();

	public String newVatitem();

	public String newVATItem();

	public String newVATItemCreated();

	public String newVendor();

	public String newVendorCreated();

	public String newVendorGroup();

	public String newZeland();

	public String next();

	public String nextNumber();

	public String nextNumberValue();

	public String nextVATQuarter();

	public String nigeria();

	public String no();

	public String noDebitorstoShow();

	public String noDepositsToShow();

	public String noDepreciationThisFinancialYear();

	public String NoEstimatesAndSalesOrderForCustomer();

	public String noEstimatesForCustomer();

	public String nominalCodeItem();

	public String none();

	public String noNegativeAmounts();

	public String noNegativeAmountsReceived();

	public String nonInventoryItem();

	public String nonTaxable();

	public String nonTaxableOther();

	public String nonVoided();

	public String noProductstoshow();

	public String noPurchaseOrderAndItemReceiptForVendor();

	public String noPurchaseOrderForSupplier();

	public String noPurchaseOrdersForVendor();

	public String noQuotesAndSalesOrderForCustomer();

	public String noQuotesForCustomer();

	public String norecordstoshow();

	public String noRecordsToShow();

	public String norecordstoshowinbetweentheselecteddates();

	public String notAdded();

	public String note();

	public String notes();

	public String notForProfitOrganization();

	public String notIssued();

	public String NOV();

	public String november();

	public String num();

	public String number();

	public String numberForAddresspadField();

	public String numberForbottomMarginField();

	public String numberForTopMarginField();

	public String OCT();

	public String october();

	public String ok();

	public String older();

	public String oldPassword();

	public String oneOfTheRequiredFieldsIsMissing();

	public String onHand();

	public String onlineBanking();

	public String onlineSalesByCustomerSummary();

	public String onlineSalesByItemSummary();

	public String onWhatbasisdoUpaySalesTaxes();

	public String open();

	public String openBalance();

	public String openCompany();

	public String openFiscalYear();

	public String openingBalance();

	public String openingBalances();

	public String openingBalanceShouldNotBeNull();

	public String openInRegister();

	public String openInvoicesbyDueDate();

	public String openSelectedItems();

	public String openVATCodeList();

	public String operating();

	public String operatingActivities();

	public String options();

	public String orderDate();

	public String orderNo();

	public String orderNumber();

	public String ordinaryIncomeOrExpense();

	public String originalAmount();

	public String other();

	public String otherAssets();

	public String otherDirectCosts();

	public String otherExpense();

	public String otherIncome();

	public String otherIncomeOrExpense();

	public String otherNominalAccounts();

	public String others();

	public String overDue();

	public String overDueInvoices();

	public String overdueInvoiceTitle();

	public String overdueValue();

	public String pageLabel();

	public String pageSetup();

	public String pageSize();

	public String pageType();

	public String paidAmount();

	public String paidWithin();

	public String pakistan();

	public String partTime();

	public String password();

	public String pastEmployee();

	public String payBalHelp();

	public String payBalSettings();

	public String payBill();

	public String PayBill();

	public String payBills();

	public String payFrom();

	public String Payfrom();

	public String payment();

	public String Payment();

	public String paymentDate();

	public String paymentDocuments();

	public String paymentFrom();

	public String paymentInformation();

	public String paymentList();

	public String paymentMethod();

	public String Paymentmethod();

	public String paymentNo();

	public String paymentReceived();

	public String payments();

	public String paymentSatus();

	public String paymentsList();

	public String paymentsReceived();

	public String PaymentsToBeIssued();

	public String paymentTerm();

	public String paymentTermDescription();

	public String paymentTermList();

	public String paymentTerms();

	public String paymentType();

	public String paypalEmail();

	public String paypalEmailHtml();

	public String paypalPayments();

	public String payrollLiability();

	public String paySalesTax();

	// -------
	public String payTo();

	public String payType();

	public String payTypeList();

	public String payVat();

	public String payVAT();

	public String payVATOwing();

	public String pendingAsset();

	public String pendingItemreceipts();

	public String pendingItemReceipts();

	public String pendingItemsList();

	public String percentage();

	public String percMargin();

	public String period();

	public String permanent();

	public String permissions();

	public String personalCareSalon();

	public String personalInstructor();

	public String petCare();

	public String phone();

	public String phoneAndFaxNumbers();

	public String phonenumber();

	public String phoneNumber();

	public String phoneText();

	public String photographer();

	public String placement();

	public String playsoundswithActions();

	public String pleaseChooseAnAccount();

	public String pleaseEnterAtleastOneTransactionForThisInvoice();

	public String pleaseEnterNewStartDate();

	public String pleaseEnterTaxGroupName();

	public String pleaseEnterUserEmail();

	public String pleaseGiveValidData();

	public String pleaseImplementMe();

	public String pleaseSelecPaymentType();

	public String pleaseSelectCustomer();

	public String pleaseSelectDate();

	public String pleaseSelectDateAfterCompanyStartDate();

	public String pleaseSelectExpenseType();

	public String pleaseSelectItemType();

	public String pleaseSelectPayFromAccount();

	public String pleaseSelectSupplier();

	public String pleaseSelectTaxType();

	public String pleaseSelectTheVendor();

	public String pleaseSelectValidVATAgency();

	public String pleaseSelectVATAgency();

	public String pleaseSelectVATAgencyAndEndingDateToViewReport();

	public String pleaseSelectVendor();

	public String pleasewaitdataisloading();

	public String point10();

	public String point11();

	public String point8();

	public String point9();

	public String PONum();

	public String portletHeaderStyle();

	public String portugal();

	public String post();

	public String postalCode();

	public String preConfigureAccountingCompany();

	public String preferredPaymentMethod();

	public String preferredShippingMethod();

	public String preferredSupplier();

	public String preferredVendor();

	public String prefferedPaymentMethod();

	public String present();

	public String presentClaims();

	public String preventPostBefore();

	public String previous();

	public String previousCalenderYear();

	public String previousClaims();

	public String previousFiscalYearSameDates();

	public String price();

	public String priceLevel();

	public String priceLevelList();

	public String primary();

	public String print();

	public String printChecks();

	public String printPreview();

	public String printVATReturn();

	public String priorVATReturns();

	public String processingRequest();

	public String product();

	public String PRoduct();

	public String productAndService();

	public String productAndServices();

	public String productGroup();

	public String productGroupName();

	public String productList();

	public String productName();

	public String profitAndLoss();

	public String profitAndLossStandard();

	public String propertyManagement();

	public String provideAtleastOne();

	public String provideAtleastOneVATRate();

	public String provience();

	public String purchaged();

	public String purchase();

	public String purchaseAnalysis();

	public String purchaseAndItemReceipt();

	public String purchaseByItemDetail();

	public String purchaseByItemSummary();

	public String purchaseByProductDetail();

	public String purchaseByProductSummary();

	public String purchaseBySupplierDetail();

	public String purchaseBySupplierSummary();

	public String purchaseByVendorDetail();

	public String purchaseByVendorSummary();

	public String purchaseClosedOrder();

	public String purchaseClosedOrderReport();

	public String purchaseDate();

	public String purchaseDescription();

	public String purchaseInformation();

	public String purchaseLiabilityAccount();

	public String purchaseOpenOrderReport();

	public String purchaseOrder();

	public String purchaseOrderDetails();

	public String purchaseOrderList();

	public String purchaseOrderNumber();

	public String purchaseOrderReport();

	public String purchaseOrders();

	public String purchasePrice();

	public String purchasePricShouldNotBeZero();

	public String purchaseTaxRate();

	public String purchaseType();

	public String qty();

	public String quantity();

	public String quickStart();

	public String quote();

	public String quoteAndSalesOrderList();

	public String quoteList();

	public String quotes();

	public String quotesandsalesOrder();

	public String quotesList();

	public String radioButtonGroup();

	public String rate();

	public String Rate();

	public String rateRangeShould0to100();

	public String rates();

	public String realEstateAgent();

	public String reasonForIssue();

	public String receivedDate();

	public String receivedFrom();

	public String receivedPayments();

	public String receivePayment();

	public String receivePayments();

	public String receiveVAT();

	public String recentActivity();

	public String recievePayments();

	public String recieveVat();

	public String reconcileAccount();

	public String recordExpenses();

	public String reduce();

	public String reducingBalance();

	public String reference();

	public String referenceNo();

	public String refresh();

	public String refunds();

	public String register();

	public String registeredAddress();

	public String registeredAsset();

	public String registeredItemsList();

	public String registeredName();

	public String reject();

	public String rejected();

	public String relatedTask();

	public String remainingTotal();

	public String remove();

	public String removeLogo();

	public String removeZeroBalances();

	public String report();

	public String reportBasis();

	public String reportBasisAccrual();

	public String reportGrid();

	public String reportGridcustomFont();

	public String reportGridHeader();

	public String reportGridheaderStyle();

	public String reportGridRow();

	public String reports();

	public String reportsHome();

	public String requiredField();

	public String restaurant();

	public String restoreDefault();

	public String retail();

	public String retainedEarnings();

	public String returnsDueOnOrBefore();

	public String reverseChargeList();

	public String reverseChargeListDetail();

	public String reverseChargeListDetailReport();

	public String reviewjournal();

	public String reviewJournal();

	public String right();

	public String rollbackDepreciation();

	public String rollBackDepreciation();

	public String rollbackDepreciationTo();

	public String salepriceExcludingTax();

	public String sales();

	public String salesAnalysis();

	public String salesAndPurchaseOrders();

	public String salesByCustomerDetail();

	public String salesByCustomerSumary();

	public String salesByCustomerSummary();

	public String salesByItemDetail();

	public String SalesByItemDetail();

	public String salesByItemSummary();

	public String SalesByItemSummary();

	public String salesByItemSummmary();

	public String salesCloseOrder();

	public String salesDescription();

	public String salesIncome();

	public String salesInformation();

	public String salesItem();

	public String salesLiabilityAccount();

	public String salesOpenOrder();

	public String salesOrder();

	public String salesOrderDetails();

	public String salesOrderList();

	public String salesorderno();

	public String salesOrderReport();

	public String salesOrders();

	public String salesperson();

	public String salesPerson();

	public String salesPersonInformation();

	public String salesPersonList();

	public String salesPersonName();

	public String salesPersons();

	public String SalesPersons();

	public String salesPersonUpdationFailed();

	public String salesPrice();

	public String salesTax();

	public String salesTaxAgency();

	public String salesTaxCollectedThisQuarter();

	public String salesTaxGroup();

	public String salesTaxGroups();

	public String salesTaxItems();

	public String salesTaxLiability();

	public String salesTaxPayable();

	public String salesTaxRate();

	public String salesType();

	public String save();

	public String saveandClose();

	public String saveAndClose();

	public String saveandNew();

	public String saveAndNew();

	public String saveButton();

	public String select();

	public String selectAll();

	public String SelectAnyOfTheFiscalYearFromList();

	public String selectaTaskNotetoseethedetails();

	public String selectATaxGroup();

	public String selectDate();

	public String selectDocument();

	public String selected();

	public String selectedFixedAssetIsInvalid();

	public String selectedTaxGroupItem();

	public String selectExpenseType();

	public String selectHowYouPaidForExpense();

	public String selectItemType();

	public String selectOneOfFollowingPayee();

	public String selectOneOfItem();

	public String selectOneType();

	public String selectPayeeType();

	public String selectPaymentMethod();

	public String selectPaymentsToIssue();

	public String selectPaymentType();

	public String selectPurchaseOrder();

	public String selectQuote();

	public String selectQuoteOrSalesOrder();

	public String selectTaxGroup();

	public String selectTaxItemFromAvailableListofTaxItems();

	public String selectTaxItemFromSelectedListofTaxItems();

	public String selectTheme();

	public String selectThemes();

	public String sell();

	public String sellingRegisteredItem();

	public String SEPT();

	public String september();

	public String service();

	public String serviceGroup();

	public String serviceMapping();

	public String serviceName();

	public String servicesOverview();

	public String setPaymentType();

	public String settings();

	public String settingsList();

	public String setUpOnlineBanking();

	public String shareHolderFunds();

	public String shippingAddress();

	public String shippingAndPostalService();

	public String shippingMethod();

	public String shippingMethodList();

	public String shippingMethodListDialog();

	public String shippingTerm();

	public String shippingTermList();

	public String shippingTerms();

	public String shipTo();

	public String show();

	public String showAllAccounts();

	public String showColumnHeadings();

	public String showHistory();

	public String showLog();

	public String showLogo();

	public String showpricesWithVAT();

	public String showRegisteredAddress();

	public String showTaxColumn();

	public String showTaxNumber();

	public String showTransactions();

	public String showUnitPrice();

	public String signInForOnlineTransfer();

	public String signUpForCreditCardProcesing();

	public String sold();

	public String soldAndDisposedItems();

	public String soldDisposedItems();

	public String sorryFailedToAdd();

	public String sorryNoHelp();

	public String sortCode();

	public String sourceName();

	public String southAfrica();

	public String spain();

	public String standard();

	public String standardcost();

	public String standardCost();

	public String standardTheme();

	public String standingOrder();

	public String startDate();

	public String startDateForManagingFixedAsset();

	public String startingCheckNo();

	public String startingChequeNo();

	public String startofFiscalYear();

	public String startOfFiscalYear();

	public String state();

	public String statement();

	public String statements();

	public String statementTitle();

	public String stateOrProvince();

	public String stateProvince();

	public String status();

	public String statuS();

	public String stockAdjustmentAccount();

	public String stockAdjustmentQuantity();

	public String stockItemAmount();

	public String stockItemExpenseAccount();

	public String stockItemIncomeAccont();

	public String stockItemName();

	public String stockItemType();

	public String stockMemo();

	public String stockSettings();

	public String stockwarehouseCombo();

	public String stopOnlineSharingTransfer();

	public String straightLine();

	public String streetAddress1();

	public String streetaddress2();

	public String streetAddress2();

	public String streetName();

	public String subAccountOf();

	public String subCategoryof();

	public String submit();

	public String submitForApproval();

	public String subTotal();

	public String supplier();

	public String supplieR();

	public String Supplier();

	public String supplierAddress();

	public String supplierBalance();

	public String supplierCredit();

	public String supplierGroup();

	public String supplieRGroup();

	public String supplierGroupList();

	public String supplierhome();

	public String supplierItemNo();

	public String supplierItems();

	public String supplierList();

	public String supplierLists();

	public String supplierMemo();

	public String supplierName();

	public String supplieRName();

	public String supplierOrderNo();

	public String supplierpay();

	public String supplierPay();

	public String supplierPayment();

	public String supplierPaymentList();

	public String supplierPayments();

	public String supplierPrePayment();

	public String supplierPrePayments();

	public String supplierProductNo();

	public String suppliers();

	public String suppliersAndPayables();

	public String supplierServiceNo();

	public String suppliersGroupList();

	public String suppliersHome();

	public String supplierSince();

	public String suppliersList();

	public String supplierTransactionHistory();

	public String supplierVatCode();

	public String support();

	public String supportLists();

	public String supportLogging();

	public String switchMaestro();

	public String switchView();

	public String switzerland();

	public String syncOnlinePayees();

	public String systemAccounts();

	public String tahoma();

	public String tax();

	public String taxable();

	public String taxableAmount();

	public String taxAccount();

	public String taxAdjustment();

	public String taxAgency();

	public String taxAgentPayment();

	public String taxAmount();

	public String taxCode();

	public String taxCodeAdjust();

	public String taxCodesList();

	public String taxCollected();

	public String taxColumn();

	public String taxDue();

	public String taxes();

	public String taxExempt();

	public String taxGroup();

	public String taxgroups();

	public String taxIncome();

	public String taxItem();

	public String taxItemName();

	public String taxItemsList();

	public String taxLine();

	public String taxNumber();

	public String taxRate();

	public String taxRateP();

	public String taxRates();

	public String taxRegNo();

	public String taxSummary();

	public String taxSummaryCashBasis();

	public String temporary();

	public String terms();

	public String termsLabel();

	public String test();

	public String thailand();

	public String themeAlreadyExists();

	public String therearenocreditstoshow();

	public String thisFieldAcceptsOnlyNumber();

	public String thisFinancialQuarter();

	public String thisFinancialYear();

	public String thisisConsideredaCashAccount();

	public String thisIsConsideredaCashAccount();

	public String thisIsConsideredACashAccount();

	public String thisIsVATInclusive();

	public String thisIsVATInclusivePrePayment();

	public String thisMonth();

	public String thisVATQuarter();

	public String thisVATQuarterToDate();

	public String thisWeek();

	public String thisWndHelpYoyManageVATTasks();

	public String timeReport();

	public String timesNewRoman();

	public String title();

	public String titleToGoHere();

	public String to();

	public String toAccount();

	public String toAddAnItemTax();

	public String toAddCreditRating();

	public String toAddCustomerGroup();

	public String toAddItemGroup();

	public String toAddOrRemoveTaxCode();

	public String toAddPayType();

	public String toAddShippingMethod();

	public String toAddShippingTerm();

	public String toAddSupplierGroup();

	public String toAddTaxGroup();

	public String toAddVendorGroup();

	public String toBePrinted();

	public String Tobeprinted();

	public String toCloseBooksOrFascalYear();

	public String toDateValidation();

	public String today();

	public String todaysSalesByCustomer();

	public String todaysSupplierPayments();

	public String toolBox();

	public String top();

	public String topMargin();

	public String topMarginValue();

	public String total();

	public String totalAmount();

	public String totalAmountToUse();

	public String totalBal();

	public String totalBalance();

	public String totalCapitalGain();

	public String totalCOGS();

	public String totalcolan();

	public String totalCreditAmount();

	public String totalCreditMemo();

	public String totalCredits();

	public String totalDebits();

	public String totalDPMC();

	public String totalEndingBalance();

	public String totalExpense();

	public String totalIncome();

	public String totalindirectCosts();

	public String totalOf();

	public String totalotherDirectCosts();

	public String totalOtherExpense();

	public String totalOtherIncome();

	public String totalOutStandingBalance();

	public String totalPrice();

	public String totalSales();

	public String totalsalesIncome();

	public String totalSalesPrice();

	public String totalVATDueBOX3();

	public String toTransferFunds();

	public String tradingAddress();

	public String tradingName();

	public String transactionAmount();

	public String transactionDate();

	public String transactionDetailByAccount();

	public String transactionDetailByTaxItem();

	public String transactionDetailsByAccount();

	public String transfer();

	public String transferFrom();

	public String transferFund();

	public String transferFunds();

	public String transferFundsOnline();

	public String transferTo();

	public String trebuchet();

	public String trialBalance();

	public String type();

	public String type1();

	public String type2();

	public String typeCompanyInfo();

	public String typeofBusiness();

	public String typeTheRateAndSelectDateIn();

	public String uk();

	public String UK();

	public String uKVAT();

	public String UKVAT();

	public String unableToFetchAccountsList();

	public String unableToGetCustomers();

	public String UnableToLoadRequiredCreditCardCharge();

	public String unableToLoadRequiredQuote();

	public String unabletoLoadTheRequiredDeposit();

	public String unCategorisedTaxAmounts();

	public String uncategorisedVATAmounts();

	public String unCategorisedVATAmountsDetail();

	public String undepositedFounds();

	public String undepositedFundsCollectedToday();

	public String underline();

	SafeHtml undoneHtml();

	public String unitofMeasure();

	public String unitPrice();

	public String unitPriceAndQuantity();

	public String unspecified();

	public String untilEndOfYear();

	public String unusedCredits();

	public String unusedPayments();

	public String update();

	public String updatedSuccessfully();

	public String updateFailed();

	public String updateSuccesful();

	public String updationFailed();

	SafeHtml uploadLogo();

	public String us();

	public String US();

	public String USA();

	public String useAccountNos();

	public String useAdjustButton();

	public String useCustomerId();

	public String user();

	public String userDetails();

	public String userDialog();

	SafeHtml userHTML();

	public String userPreference();

	public String users();

	SafeHtml usersComment();

	public String usersTitle();

	public String useVendorId();

	public String usLetter();

	public String value();

	public String vat();

	public String VAT();

	public String vat100();

	public String VAT100();

	public String vAT3Ireland();

	public String VATAccount();

	public String vatAdjustment();

	public String VATAdjustment();

	public String VATAdjustmentSuccessfull();

	public String vatAgency();

	public String VatAgency();

	public String VATAgency();

	public String vATAmount();

	public String VATAmount();

	public String vatBalance();

	public String vatCode();

	public String VATCode();

	public String vatCodeList();

	public String VATCodeList();

	public String VATCodeUpdatedSuccessfully();

	public String vatDetail();

	public String vatDetails();

	public String vatDue();

	public String VATFraction();

	public String vatGroup();

	public String VATGroup();

	public String vatgroupList();

	public String VATGroupList();

	public String VATGroupUpdatedSuccessfully();

	public String vatItem();

	public String VATItem();

	public String vatItemDetail();

	public String VATItemDetailReport();

	public String VATItemForPurchases();

	public String VATItemForSales();

	public String vatitemList();

	public String VATItemName();

	public String VATItemsList();

	public String vatItemSummary();

	public String VATItemSummary();

	public String VATItemUpdatedSuccessfully();

	public String VATLine();

	public String vatList();

	public String VATPayment();

	public String vatRate();

	public String vATRate();

	public String VATRateInPerc();

	public String VATRateShouldRange0to100();

	public String vatRegistrationNumber();

	public String VATRegistrationNumber();

	public String vatRegNo();

	public String VATReturn();

	public String VATReturnBox();

	public String vatSummary();

	public String venderForm();

	public String vendor();

	public String vendoR();

	public String Vendor();

	public String vendor1099();

	public String vendorAddress();

	public String Vendorbalance();

	public String vendorBill();

	public String vendorCredit();

	public String vendorCreditMemo();

	public String vendorGroup();

	public String vendorGroupList();

	public String vendorHome();

	public String vendorItems();

	public String vendorList();

	public String vendorLists();

	public String vendorMemo();

	public String vendorName();

	public String vendoRName();

	public String vendorOrderNo();

	public String vendorpay();

	// ---------
	public String vendorPayment();

	// ------------
	public String vendorPaymentIssued();

	public String Vendorpaymentmethod();

	public String vendorPayments();

	public String vendorPaymentsList();

	public String vendorPrePayment();

	public String vendorPrePayments();

	public String vendorProductNo();

	public String vendors();

	public String vendorsAndPayables();

	public String vendorServiceNo();

	public String vendorServices();

	public String vendorsHome();

	public String vendorSince();

	public String vendorsList();

	public String vendorTransactionHistory();

	public String vendorUpdatedSuccessfully();

	public String vendorVatCode();

	public String vertical();

	public String view();

	public String viewDisposalJournal();

	public String viewEdit();

	public String viewSalesTaxLiability();

	public String viewVATItems();

	public String Void();

	public String voided();

	public String Voided();

	public String voidedCashBasis();

	public String voucherNo();

	public String voucherType();

	public String warehouse();

	public String wareHouse();

	public String warehouseList();

	public String wareHouseTitle();

	public String wareHouseTransfer();

	public String warehouseTransferList();

	public String wareName();

	public String warnOfFascalYear();

	public String webBasedSales();

	public String webFormatExpr();

	public String webPageAddress();

	public String website();

	public String webSite();

	public String weight();

	public String welcome();

	public String WELCOMEBANKINGSUMMARY();

	public String whowillrunwhenaccoutantTransfer();

	public String widgetsAlreadyOnYourHomepageLabel();

	public String widgetsAvailable();

	public String width();

	public String writeCheck();

	public String writeCheckView();

	public String writeLetters();

	public String writeOff();

	public String writeOffAccount();

	public String writeOffAmount();

	public String WriteOffPleaseAddDetails();

	public String year();

	public String year2011();

	public String year2012();

	public String yearToDate();

	public String yes();

	public String youCannotEdiClosedFiscalYear();

	public String youCanPrintVATReturn();

	public String youCantDelete();

	public String youCantDeleteItem();

	public String youCantDeleteThisAccount();

	public String youCantDeleteThisCustomer();

	public String youCantSetTypeOfObject();

	public String youCantVoidThisOne();

	public String youHaveVATRefundOf();

	public String youMustEnterBankName();

	public String yourTitle();

	public String ytdSalesComparedToLastYear();

	public String zipcode();

	public String zipCode();

	public String zipPostalCode();
}
