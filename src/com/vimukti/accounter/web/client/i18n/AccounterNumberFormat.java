 ...
 * <td>pattern</td>
 * <td>:=</td>
 * <td style="white-space: nowrap">subpattern ('<code>;</code>' subpattern)?</td>
 * </tr>
 * <tr>
 * <td>subpattern</td>
 * <td>:=</td>
...
public class AccounterNumberFormat {
	...
	/**
	 * Specify whether all new NumberFormat instances will use latin digits and
	 * related characters rather than the localized ones.
	 * 
	...
	protected static NumberConstants createLatinNumberConstants(
		...
		final String groupingSeparator = remapSeparator(orig
				.groupingSeparator());
		final String decimalSeparator = remapSeparator(orig.decimalSeparator());
		final String monetaryGroupingSeparator = remapSeparator(orig
				.monetaryGroupingSeparator());
		final String monetarySeparator = remapSeparator(orig
				.monetarySeparator());
	...
	public AccounterNumberFormat(String pattern, int numberOfDecimals,
		...
		// TODO: handle per-currency flags, such as symbol prefix/suffix and
		// spacing
	...
	public double parse(String text, int[] inOutPos)
		...
		boolean gotPositivePrefix = text
				.startsWith(positivePrefix, inOutPos[0]);
		boolean gotNegativePrefix = text
				.startsWith(negativePrefix, inOutPos[0]);
		...
		if (gotPositive) {
			...
			valueOnly = text.substring(inOutPos[0], text.length()
