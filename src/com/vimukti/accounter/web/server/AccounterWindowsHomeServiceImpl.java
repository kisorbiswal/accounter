...
public class AccounterWindowsHomeServiceImpl extends
	...
	@Override
	public void selectComapny(long companyId) throws StartupException {
		Session openSession = HibernateUtil.getCurrentSession();
		// TODO need more validation check.
		try {
			Company comapny = (Company) openSession.get(Company.class,
					companyId);
			if (comapny != null) {
				getThreadLocalRequest().getSession().setAttribute("companyId",
						companyId);
				// getThreadLocalRequest().getSession().setAttribute("isTouch",
				// getThreadLocalRequest().getAttribute("isTouch"));
			} else {
				getThreadLocalRequest().getSession().setAttribute(CREATE,
						"true");
			}
