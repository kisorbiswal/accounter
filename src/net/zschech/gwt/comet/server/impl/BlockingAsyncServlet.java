...
public class BlockingAsyncServlet extends AsyncServlet {
	...
	@Override
	public Object suspend(CometServletResponseImpl response,
			CometSessionImpl session, HttpServletRequest request)
			throws IOException {
		...
		if (session == null) {
			try {
				synchronized (response) {
					while (!response.isTerminated()) {
						long heartBeatTime = response
								.getHeartbeatScheduleTime();
						if (heartBeatTime <= 0) {
							try {
								response.heartbeat();
							} catch (IOException e) {
			...
			try {
				try {
					synchronized (response) {
						...
						while (session.isValid() && !response.isTerminated()) {
							long sessionKeepAliveTime = session
									.getKeepAliveScheduleTime();
							if (sessionKeepAliveTime <= 0) {
								if (access(session.getHttpSession())) {
									session.setLastAccessedTime();
									sessionKeepAliveTime = session
											.getKeepAliveScheduleTime();
								} else {
									response.tryTerminate();
									break;
							...
							if (session.isEmpty()) {
								...
								response.wait(Math.min(sessionKeepAliveTime,
										heartBeatTime));
								response.setProcessing(true);
							} else {
				...
				log("Error writing messages - IllegalStateException");
			}
	...
	@Override
	public void terminate(CometServletResponseImpl response,
			final CometSessionImpl session, boolean serverInitiated,
			Object suspendInfo) {
		assert Thread.holdsLock(response);
		response.notifyAll();
	}
