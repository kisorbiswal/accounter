...
public abstract class CometServletResponseImpl implements CometServletResponse {
	...
	protected CometServletResponseImpl(HttpServletRequest request,
	...
	public synchronized CometSession getSession(boolean create) {
		if (suspended) {
			throw new IllegalStateException(
					"CometSession can not be accessed after the CometServletResponse has been suspended.");
		}
		if (session != null) {
			return session;
		}
		...
		session = (CometSessionImpl) CometServlet.getCometSession(httpSession,
				create);
		if (create) {
	...
	@Override
	public synchronized void sendError(int statusCode, String message)
			throws IOException {
		if (suspended) {
			throw new IllegalStateException(
					"sendError can not be accessed after the CometServletResponse has been suspended.");
		}
		try {
			response.reset();
			...
			servlet.log("Error resetting response to send error: "
					+ e.getMessage());
		} finally {
			setTerminated(true);
		}
	...
	void tryTerminate() {
		try {
			...
			servlet.log("Error terminating response");
		}
	}
	...
	@Override
	public synchronized void write(List<? extends Serializable> messages,
	...
	void tryHeartbeat() {
		try {
			...
			servlet.log("Error sending heartbeat");
		}
	}
	...
	void setTerminated(boolean serverInitiated) {
		...
		if (serverInitiated) {
			try {
				...
				servlet.log("Error closing connection");
			}
		}
	...
	protected String serialize(Serializable message)
			throws NotSerializableException, UnsupportedEncodingException {
		try {
			...
			throw new NotSerializableException(
					"Unable to serialize object, message: " + e.getMessage());
		}
	}
